...
# 브레이크아웃 예제에서의 DDQN 에이전트
class DDQNAgent:
    def __init__(self, action_size, state_size=(84, 84, 4), model_path_=None, memory=None,epsilon=None):
        self.render = False
        ...
        self.exploration_steps = 1200000.
    ...
    def train_model(self):
        ...
        with tf.GradientTape() as tape:
            ...
            main_predicts = self.model(next_history)
            target_predicts = self.target_model(next_history)
            target_q_val = np.zeros(self.batch_size)

            # DDQN 타겟함수를 통해 타겟 q 계산
            for i in range(np.array(main_predicts).shape[0]):
                max_q_arg = np.argmax(np.array(main_predicts[i,:]))
                target_q_val[i] = np.array(target_predicts)[i, max_q_arg]
            targets = rewards + (1 - dones) * self.discount_factor * target_q_val
...
if __name__ == "__main__":
    ...
    for e in range(num_episode):
        ...
        if e % 200 == 0:
            ...
            np.savez('./score/dDdqn_score.npz', score_board=score_board)
            np.savez('./param_memory/dDdqn_param_memory.npz', memory=agent.memory, epsilon=agent.epsilon)
